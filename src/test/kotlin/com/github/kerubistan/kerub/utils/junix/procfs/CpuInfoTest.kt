...
class CpuInfoTest {
	...
	fun listOnPpc() {
		...
		whenever(sftp.read(eq("/proc/cpuinfo"))).thenReturn(
				resource("com/github/kerubistan/kerub/utils/junix/procfs/cpuinfo-linux-power.txt")
		)
	...
	fun list() {
		whenever(session.createSftpClient()).thenReturn(sftp)
		whenever(sftp.read(eq("/proc/cpuinfo"))).thenReturn(
				resource("com/github/kerubistan/kerub/utils/junix/procfs/cpuinfo-linux-i7.txt")
		)
